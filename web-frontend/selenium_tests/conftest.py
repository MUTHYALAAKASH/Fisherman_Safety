...
def driver():
    ...
    # Enable microphone/location access auto-grants, disable popups
...
def pytest_sessionfinish(session, exitstatus):
    ...
    title_cell.value = "Smart Fisherman Safety Web - E2E Execution Dashboard"
    ...
    ws_summary["B3"] = "Smart Fisherman Safety Web Portal"
    ...
    # Save Excel workbook
    report_name = "selenium_test_report.xlsx"
    try:
        wb.save(report_name)
        print(f"\n[Selenium Excel Reporter] Multi-sheet execution report saved to: {report_name}")
    except PermissionError:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        fallback_name = f"selenium_test_report_{timestamp}.xlsx"
        try:
            wb.save(fallback_name)
            print(f"\n[WARNING] Could not save to '{report_name}' because the file is open/locked in another program.")
            print(f"[Selenium Excel Reporter] Saved fallback report to: {fallback_name}")
